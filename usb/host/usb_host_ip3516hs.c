...
{
    usb_host_ip3516hs_state_struct_t * usbHostState = (usb_host_ip3516hs_state_struct_t *)
                                                      ((usb_host_instance_t *)hostHandle)->controllerHandle;
    ...
    delayUs(5);
    USB_HostIp3516HsTestSetMode(usbHostState, 0);
}
                                                       ...
                                                       usb_host_transfer_t *tr)
{
    ...
    /* uFrame[2:0] defines the polling rate. If set to 0 and the transaction is high-speed, 
    the uSA determines during which uFrames a packet can be sent. */
    ptl->control1Union.stateBitField.uFrame = (insertUFrame & 0xF8U); // clear uFrame[2:0] bits
    ...
    if (USB_SPEED_HIGH == speed)
    {
        for (int i = ptl->control1Union.stateBitField.uFrame & 0x07U; i < 8; i += pipe->pipeCommon.interval)
        {
            ptl->statusUnion.statusBitField.uSA |= 1U << i;
        }
    }
