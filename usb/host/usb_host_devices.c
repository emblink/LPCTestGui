            ...
            if (unionDes->common.bDescriptorType == USB_DESCRIPTOR_TYPE_INTERFACE)
            {
                if (unionDes->interface.bAlternateSetting == 0x00)
                {
                    ...
                    if (interfaceParse->interfaceDesc->bNumEndpoints != 0)
                    {
                        ...
                        g_HostHidGeneric[interfaceParse->interfaceIndex].endpointCount = interfaceParse->interfaceDesc->bNumEndpoints;
                             ...
                             (interfaceParse->epCount)++)
                        {
                            ...
                            g_HostHidGeneric[interfaceParse->interfaceIndex].endpoint[interfaceParse->epCount].epAddress = epParse->epDesc->bEndpointAddress;
                            g_HostHidGeneric[interfaceParse->interfaceIndex].endpoint[interfaceParse->epCount].bInterval = epParse->epDesc->bInterval;
                            g_HostHidGeneric[interfaceParse->interfaceIndex].endpoint[interfaceParse->epCount].epMaxPacketSize =
                            (uint16_t) USB_SHORT_FROM_LITTLE_ENDIAN_ADDRESS(epParse->epDesc->wMaxPacketSize);
